...
map:
    ...
    # THINK: Why a0? Ans: Because we have to call the function f so we need to pass the value to f using a0
    ### YOUR CODE HERE ###
    lw a0, 0(s0)

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why). Ans: Because map will be hardcoded
    #to call only that label no matter which function pointer we pass
    # Hint: Where do we keep track of the function to call? Recall the parameters of "map". Ans: s1 or according to parameters, a1
    ### YOUR CODE HERE ###
    jalr ra, s1, 0
    ...
    # THINK: why a1? What about a0? Ans: because a0 is already used to store the address of the next/head node to be processed
    ### YOUR CODE HERE ###
    add a1, s1, zero  # No need of this instruction because a1 isn't changed throughout the whole map function
    # Recurse
    ### YOUR CODE HERE ###
    jal ra, map
